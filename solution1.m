...
for i = 1:1:length(scores)
     if idx(i) == 1
        scores_cls1(index1, 1) = scores(i, 1);
        index1 = index1+1;
    elseif idx(i) == 2
        scores_cls2(index2, 1) = scores(i, 1);
        index2 = index2+1;
     end
end
z = zscore([cls1_0; cls1_1]);
x1 = z(:, 1);
x2 = z(:, 2);
X = [ones(size([cls1_0; cls1_1])), x1, x2, x1.^2, x2.^2, x1.*x2];
...
z = zscore([cls2_0; cls2_1]);
x1 = z(:, 1);
x2 = z(:, 2);
X = [ones(size([cls2_0; cls2_1])), x1, x2, x1.^2, x2.^2, x1.*x2];
